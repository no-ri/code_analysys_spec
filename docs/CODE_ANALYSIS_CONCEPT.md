...
**絶対に守る4つのルール**（**ルール4 は 2026-08-31 に追加**。番号1〜3 は参照が多数あるため動かさない）:
...
4. **解析側とアプリ側を L1 で切り、アプリは後から足せる形にする**（2026-08-31 追加）
   **L1（ファクト層）を唯一の境界とし、その上下を独立に差し替えられるようにする。**

   | | 中身 | 変わる理由 |
   |---|---|---|
   | **解析側**（L0 / L1 の生成） | 抽出器、解決規則 | 言語が増える、規則の精度が上がる（F-16 で `base.` の扱いを直したように） |
   | **アプリ側**（L2 / L3a / L3b） | 図、レポート、検索、ルール | **実際のデータを見て「何を読みたいか」を思いつく** |

   **アプリ側は初版で決め打ちにしない。** L3a の出力（図・レポート）は**プラグインとして追加できる形**にし、
   本体を触らずに増やせるようにする。L3b のルールについては既に「プラグイン形式」と決めている（下記「別リポジトリにすべきか」）が、
   **同じ扱いを L3a にも適用する**。

   **理由**: 取得したデータから何を読み出すかは、**実際のデータを見て初めて思いつく**。
   最初から決め打ちの設計にすると、**解析を自作している意味が無くなる**——
   出来合いのツールを使わずに自前でデータを持つのは、後から自由に問いを立てられるようにするためである。

   E-1 の5レポートは**初版に入れる最小セットであって、出せるものの上限ではない**。

   > **アプリ側だけでなく解析側も変わる。** 双方向に効くルールである。
   > 解析側が変わってもアプリ側が壊れないのは、L1 のスキーマが固定されているから（`SCHEMA.md` が正本）。
   > アプリ側が増えても解析側が汚れないのは、ルール1・2 が L0 に判断を入れさせないから。
   > **`SCHEMA.md` を正本に一本化したこと（F-1）が、この分離の前提条件になっている。**

   これは **B-3**（「ロジックは `core/`、CLI は薄いラッパー、ジョブ設定ファイル、構造化進捗出力」で
   GUI を後から安く足せるようにする）の延長にあたる。B-3 が**実行の入口**を後付け可能にしたのに対し、
   ルール4 は**出力の出口**を後付け可能にする。
...
> **ルール4 により、これは L3b のルールだけの話ではない。** L3a の図・レポートも同じくプラグイン形式にする。
> 「1レポート1ファイル、入力は L1 への SQL、出力は成果物」という形にしておけば、
> **レポートの追加・削除・差し替えが本体にも L1 にも触れずに済む。**
> 実現手段（Mermaid / 自作 HTML / 別形式）の決定は `OPEN_DECISIONS.md` の C-2。
